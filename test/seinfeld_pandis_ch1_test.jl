...
@testitem "2.3 Eq. 1.5: Barometric Formula" begin
    ...
    p_at_H = p_0 * exp(-H / H)
    @test p_at_H ≈ p_0 / exp(1) rtol = 1.0e-10

    # At z = 0, pressure should be p_0
    p_at_0 = p_0 * exp(-0 / H)
    @test p_at_0 ≈ p_0 rtol = 1.0e-10
...
@testitem "2.5 Eq. 1.6: Volume Mixing Ratio" begin
    ...
    xi_O3 = 40.0e-9   # 40 ppb

    c_O3 = xi_O3 * c_total
    @test c_O3 ≈ 1.78e-6 rtol = 0.01

    # Verify inverse: xi = c_i / c_total
    @test c_O3 / c_total ≈ xi_O3 rtol = 1.0e-10
end
...
@testitem "2.6 Eq. 1.8: Mixing Ratio and Partial Pressure" begin
    ...
    # Verify inverse
    @test p_CO2 / p_total ≈ xi_CO2 rtol = 1.0e-10
end
...
@testitem "2.7 Eq. 1.10: Saturation Vapor Pressure" begin
    ...
    T1 = 373.15
    a1 = 1 - T_boil / T1
    @test a1 ≈ 0.0 atol = 1.0e-10
    p_sat_1 = p_std * exp(13.3185 * a1 - 1.976 * a1^2 - 0.6445 * a1^3 - 0.1299 * a1^4)
    @test p_sat_1 ≈ 1013.25 rtol = 1.0e-10

    # Test at T = 273 K: p_sat ~ 6.1 mbar (from specification)
    T2 = 273.15
    a2 = 1 - T_boil / T2
    p_sat_2 = p_std * exp(13.3185 * a2 - 1.976 * a2^2 - 0.6445 * a2^3 - 0.1299 * a2^4)
    @test p_sat_2 ≈ 6.1 rtol = 0.05

    # Test at T = 298 K: p_sat ~ 31.7 mbar (from specification)
    T3 = 298.15
    a3 = 1 - T_boil / T3
    p_sat_3 = p_std * exp(13.3185 * a3 - 1.976 * a3^2 - 0.6445 * a3^3 - 0.1299 * a3^4)
...
@testitem "2.8 Eq. 1.9: Relative Humidity" begin
    ...
    p_H2O = 0.5 * p_sat
    RH = 100 * p_H2O / p_sat
    @test RH ≈ 50.0 rtol = 1.0e-10

    # At saturation (100% RH)
    p_H2O_sat = p_sat
    RH_sat = 100 * p_H2O_sat / p_sat
    @test RH_sat ≈ 100.0 rtol = 1.0e-10
end
...
@testitem "4.4 Saturation Vapor Pressure Increases with Temperature" begin
    ...
    calc_psat = (T) -> begin
        a = 1 - T_boil / T
        p_std * exp(13.3185 * a - 1.976 * a^2 - 0.6445 * a^3 - 0.1299 * a^4)
    end
...
@testitem "6.1 Physical Constants" setup = [SPCh1Setup] begin
    ...
    @test AtmosphericDynamics.R_GAS ≈ 8.314 rtol = 0.001
    @test AtmosphericDynamics.M_AIR ≈ 0.02897 rtol = 0.001
    @test AtmosphericDynamics.G_ACCEL ≈ 9.807 rtol = 0.001
    @test AtmosphericDynamics.P_0 ≈ 101325 rtol = 0.001
    @test AtmosphericDynamics.T_0 ≈ 273.15 rtol = 0.001
    @test AtmosphericDynamics.T_BOIL ≈ 373.15 rtol = 0.001
    @test AtmosphericDynamics.P_STD_MBAR ≈ 1013.25 rtol = 0.001
end
